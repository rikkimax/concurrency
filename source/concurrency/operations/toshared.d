...
auto toShared(Sender, Scheduler)(Sender sender, Scheduler scheduler) {
	return new SharedSender!(Sender, Scheduler,
	                         ResetLogic.keepLatest)(sender, scheduler);
}

auto toShared(Sender)(Sender sender) {
	return new SharedSender!(Sender, NullScheduler,
	                         ResetLogic.keepLatest)(sender, NullScheduler());
}
...
class SharedSender(Sender, Scheduler, ResetLogic resetLogic) {
	...
	auto connect(Receiver)(return Receiver receiver) @safe return scope {
		// ensure NRVO
		auto op = SharedSenderOp!(Sender, Scheduler, resetLogic,
...
private
struct SharedSenderOp(Sender, Scheduler, ResetLogic resetLogic, Receiver) {
	alias Props = Properties!(Sender);
...
private class SharedSenderInstStateImpl(
	Sender,
	Scheduler,
	ResetLogic resetLogic
) : SharedSenderInstState!(Sender) {
	alias Op =
		OpType!(Sender, SharedSenderReceiver!(Sender, Scheduler, resetLogic));
	Op op;
}
